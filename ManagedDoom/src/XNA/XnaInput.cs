...
namespace ManagedDoom.Xna
{
    public sealed class XnaUserInput : IUserInput, IDisposable
    {
        ...
        public XnaUserInput(XnaDoom xnaDoom, Config config, bool useMouse)
        {
            try
            {
                ...
                mouseGrabbed = false;
                windowCenterX = xnaDoom.GraphicsDevice.PresentationParameters.BackBufferWidth / 2;
                windowCenterY = xnaDoom.GraphicsDevice.PresentationParameters.BackBufferHeight / 2;
                mouseX = 0;
                mouseY = 0;
        ...
        public void BuildTicCmd(TicCmd cmd)
        {
            ...
            var ms = 0.75F * config.mouse_sensitivity;
        ...
        public void ReleaseMouse()
        {
            if (useMouse && mouseGrabbed)
            {
                var posX = (int)(0.9 * xnaDoom.GraphicsDevice.PresentationParameters.BackBufferWidth);
                var posY = (int)(0.9 * xnaDoom.GraphicsDevice.PresentationParameters.BackBufferHeight);
                Mouse.SetPosition(posX, posY);
                //window.SetMouseCursorGrabbed(false);
                xnaDoom.IsMouseVisible = true;
                mouseGrabbed = false;
            }
        }
        ...
        private void UpdateMouse(MouseState mouseState)
        {
            if (mouseGrabbed)
            {
                if (cursorCentered)
                {
                    var currentX = mouseState.X;
                    var currentY = mouseState.Y;

                    mouseX = currentX - windowCenterX;
                ...
                cursorCentered = (newState.X == windowCenterX && newState.Y == windowCenterY);
            }
            else
            {
                mouseX = 0;
        ...
        {
            for (var i = (int)Keys.Back; i <= (int)Keys.OemClear; i++)
            {
                var key = (Keys)i;

                if (keyboardState[key] == KeyState.Down && previousKeyboardState[key] == KeyState.Up)
                {
                    xnaDoom.PostEvent(new DoomEvent(EventType.KeyDown, XnaToDoom(key)));
                }

                if (keyboardState[key] == KeyState.Up && previousKeyboardState[key] == KeyState.Down)
                {
                    xnaDoom.PostEvent(new DoomEvent(EventType.KeyUp, XnaToDoom(key)));
                }
            }
        }
